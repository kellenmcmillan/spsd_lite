...
var AppDataModel = require('./app_data');
...
function App(config) {
  ...
  this.app_data = AppDataModel(this.connections.db);
  this.User = UserModel(this.connections.db);
  this.Permissions = authModel(this.connections.db);
...
// Add Jobs To Task Queue From API Endpoints
App.prototype.api_endpoint_add_task_to_queue = function(request, response, next) {
  var id = uuid.v1();
  queue.publish(API_TASK_DATABASE_QUEUE, { id: request.id, conditions: request.conditions, taskName: request.taskName });
  return Promise.resolve(id);
};
// Add Jobs To Task Queue From API Endpoints

// Add Jobs To Messaging Task Queue 
App.prototype.long_running_message_task_queue = function(task) {
  var id = uuid.v1();
  queue.publish(MESSAGING_QUEUE, { id: id, conditions: conditions, taskName: taskName });
  return Promise.resolve(id);
};
// Add Jobs To Messaging Task Queue

// Add Jobs To Messaging Task Queue 
App.prototype.short_running_message_task_queue = function(task) {
  var id = uuid.v1();
  queue.publish(MESSAGING_QUEUE, { id: id, conditions: conditions, taskName: taskName });
  return Promise.resolve(id);
};
...
// Add Jobs To App Data Upload Task Queue 
App.prototype.save_data_task_queue = function(task) {
  var id = uuid.v1();
  queue.publish(SAVE_DATA_QUEUE, { id: id, conditions: conditions, taskName: taskName });
  return Promise.resolve(id);
};
// Add Jobs To App Data Upload Task Queue

// Get App Data Using Schema Static
App.prototype.get_app_data = function() {
  return this.app_data.get();
};
// Get App Data Using Schema Static
...
// Find Permissions Using Schema Static
App.prototype.get_permissions_config = function() {
...
//Job Handler
App.prototype.process_media = function(job, ack) {
...
//Job Handler
// App.prototype.save_data = function(job, ack) {
