...
$window){
    ...
    this.delete_images = function(arg){
        ...
        realtimeDatabase.ref().child('media/images').child(arg.id).update(null, function(error){
            ...
            return new Promise(function (resolve, reject) {
                ...
                task.on('state_changed',
                    ...
                    function complete(){
                        ...
                        realtimeDatabase.ref().child('users').child($rootScope.me.id + "/files/" + fileObj.id).set(fileObj, function(error){
                            if (error){
                                $timeout(function(){
                                   $rootScope.$broadcast('server-event', {
                                        data:{
    ...
    this.delete_my_files = function(arg, argFile){
        realtimeDatabase.ref().child('users').child(arg.id).child("files/" + argFile.id).update(null, function(error){
            if (error){
    ...
    this.modify_user = function(arg){
        ...
        realtimeDatabase.ref().child('users').child(arg.id).update(update, function(error){
            if (error){
                // error logging
            } else {
