...
$window){
    ...
    this.check_tasks = function(e){
        ...
        .then(function(snapshot) {
            ...
            $rootScope.sort_lightweight_interaction_tasks();
        }); 
    }
    ...
    this.run_task = function(arg, event){
        var uid = $rootScope.me.id ? $rootScope.me.id : undefined;
        var ip = event.ip ? event.ip : undefined;
        ...
        console.log("Data package to send to API: " + JSON.stringify(data));
        ...
        $rootScope.set_event_object({
            "id": $rootScope.me.id ? $rootScope.me.id : undefined,
            "ip": event.ip ? event.ip : undefined,
            "timestamp": Date.now(), 
            "action": undefined,
        });

        $rootScope.reset_event({"data": "An app interaction trigger fired"});
    }

    this.save_event = function(task, event){

        var executable_event = {

            firstname: $rootScope.me.firstname ? $rootScope.me.firstname : undefined,
            lastname: $rootScope.me.lastname ? $rootScope.me.lastname : undefined,
            phone: $rootScope.me.phone ? $rootScope.me.phone : undefined,
            email: $rootScope.me.email ? $rootScope.me.email : undefined,
            uid: $rootScope.me.id ? $rootScope.me.id : undefined,
            ip: event.ip ? event.ip : undefined,
            action : event.action,
            event : event,
            task: task,
        ...
        console.log("Event data to save to database: " + JSON.stringify(executable_event));

        realtimeDatabase.ref('/data/events').child(executable_event.id).set(executable_event);
...
$http){
    ...
    $rootScope.sort_lightweight_interaction_tasks = function(){
    ...
    $rootScope.reset_event_object = function(obj){
        $rootScope.event_object = obj;
        // restart periodic event logging
        logger = $interval(lightweight_log, 10000);
        analyze = $interval(analyze_lightweight_log, 10000);
        // restart periodic event logging
    }

    // Handler for event reset
    $rootScope.reset_event = function(e){
        // e.data comes from button
        // e.action comes from button
        e.clickpath = clickpath;
        e.id = $rootScope.me ? $rootScope.me.id : undefined;
        e.ip = ip_for_events;
        ...
        $rootScope.reset_event_object(e);
    }
    // Handler for event reset
    ...
    $rootScope.set_event_object({
        "clickpath": clickpath, 
        "id": $rootScope.me ? $rootScope.me.id : undefined,
        "ip": ip_for_events,
        "data": undefined,
        "timestamp": Date.now(),
        "page": $location.path(), 
    ...
    var analyze_lightweight_log = function(){
        var e = lightweight_firebase.log_events(null, "get");
        var interactions = $rootScope.interactions;
        if (e.action === interactions[0].task.user_action_trigger){
            console.log("event to task match");
            $interval.cancel(logger);
            $interval.cancel(analyze);
            return lightweight_firebase.save_event(interactions[0].task, e);
        } else {
            return;
        }
    }
    ...
    var logger = $interval(lightweight_log, 10000);
    var analyze = $interval(analyze_lightweight_log, 10000);
    // start periodic event logging
