...
$window){
    ...
    this.run_task = function(arg, event){
        ...
        } else if(event.action === arg.task_trigger) {

            var uid = $rootScope.me ? $rootScope.me.id : undefined;
            var ip = event.ip ? event.ip : undefined;
            var action = arg.task_trigger;
            uid = JSON.stringify(uid);
            ip = JSON.stringify(ip);
            console.log("this is the task trigger: " + action);

            console.log("this is the event: " + event.action);
...
$http){
    ...
    $rootScope.sort_lightweight_interaction_tasks = function(){
        ...
        console.log("Tasks With Triggers: " + JSON.stringify($rootScope.interactions));
    }
    // get app interaction tasks
    ...
    $rootScope.$on('app-event', function(event, args) {
        set_event_object({
            "clickpath": clickpath, 
            "id": $rootScope.me ? $rootScope.me.id : undefined,
            "ip": ip_for_events,
    ...
    var analyze_lightweight_log = function(){
        ...
        console.log("Event: " + e.action + ", Interaction: " + interactions[0].task_trigger);
        if (e.action === interactions[0].task_trigger){
            console.log("App Interaction Based Event Triggered " + Date.now());
