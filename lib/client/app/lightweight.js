...
$window){
    ...
    this.sign_up = function(arg){
        ...
        // firebase method to sign up with username password
        $rootScope.auth.$createUserWithEmailAndPassword(arg.email, arg.password)
        .then(function(snap) {
            ...
            if(snap.uid){
                ...
                var newUserAccount = {};
                newUserAccount.security = {};
                newUserAccount.security.roles = 'basic';
                newUserAccount.email = snap.email;
                newUserAccount.id = snap.uid;

                console.log(JSON.stringify(newUserAccount));

                // Store user in database
                var newSignupUser = realtimeDatabase.ref().child("/users/" + snap.uid).set(newUserAccount);
                newSignupUser.once("value")
                .then(function(snapshot) {
                    ...
                    // create and save data to events database for messaging consumption
                    e.payload = childData;
                    e.action = "create-external-account";
                    e.task_id = "lw005";
                    e.clickpath = $rootScope.clickpath;
                    e.user_id = childKey;
                    e.user_ip = $rootScope.ip_for_events;
                    e.name = "New User Signup";
                    ...
                    // Redirect user to homepage after login via signup
                    $timeout(function(){
                ...
                // if error retrieving user value from newly created data
                .catch(function(error) {
                    $timeout(function(){
                        $rootScope.$broadcast('server-event', {
                            data:{
