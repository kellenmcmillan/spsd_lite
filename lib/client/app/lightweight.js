...
$http){
    ...
    var event_object = {};
    var clickpath = [];
    var user_id = $rootScope.me ? $rootScope.me.id : null;
    // Lightweight Event Logging Method
    var lightweight_log = function(){
        lightweight_firebase.log_events(event_object, "set");
        event_object = {};
    }
    ...
    $rootScope.$on('$routeChangeSuccess', function() {
        clickpath.push($route.current);
        if (!app_is_loaded){
            set_event_object({"clickpath": clickpath , "id": user_id, "item": null, "page": $route.current, "action": "Initial App Load"});
            app_is_loaded = true;
        } else {
            set_event_object({"clickpath": clickpath , "id": user_id, "item": null, "page": $route.current, "action": null});
        }
    });
