...
var spotlight = angular.module('spotlight', [
'ngResource', 
'ngAnimate', 
'ngSanitize'
])
...
$routeParams)
{
    ...
    var load_page = function(){
        for(let i = 0, l = $rootScope.pages.length; i < l && continue_looping == true; i++) {
            ...
            var checkURL = raw_url.split("/spotlight/").pop();
            var url = JSON.stringify(checkURL);
            if(url == pageId){
                $('#default-page-loading').fadeOut('slow');
                continue_looping = false;
                // $rootScope.pages[i].pageNumber = i;
                $rootScope.spotlightPage = $rootScope.pages[i];
                $rootScope.pageTitle = $rootScope.spotlightPage.data.title;
                $rootScope.spotlightPageSortObj = [];
                var spotlightPageSortableContents = angular.forEach($rootScope.spotlightPage.data.page.contents, function(value, key){
                    $rootScope.spotlightPageSortObj.push(value);
                });
                $rootScope.spotlightPageSortableContents = spotlightPageSortableContents;
            }
        }
    }
    init();
