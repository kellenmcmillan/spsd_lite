...
fromGalleriesDatabase
    ...
    $rootScope.open_menu = function(item){
        ...
        $rootScope.sub_navigation_panel = true;
    ...
    $rootScope.open_parent_menu = function(){
        $rootScope.navigation_panel = true;
    };

    $rootScope.close_parent_menu = function(){
        $rootScope.navigation_panel = null;
    };
    ...
    imageData.getImages().then(function(result){
    ...
    folderData.getFolders().then(function(result){
        if(result.length > 0){
...
.factory('folderData', function($q){
    var realtimeDatabase = firebase.database();
    var folders = realtimeDatabase.ref().child('folders');
    var getFolders = function(){
        ...
        .then(function(snapshot) {
            snapshot.forEach(function(childSnapshot) {
                ...
                var childData = childSnapshot.val();
                var childDataValue = childData;
                folderList.push(childDataValue);
            });
