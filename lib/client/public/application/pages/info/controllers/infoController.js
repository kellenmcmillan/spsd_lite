...
$routeParams)
{
    ...
    var load_page = function(){
        for(let i = 0, l = $rootScope.pages.length; i < l && continue_looping == true; i++) {
            ...
            var checkURL = raw_url.split("/info/").pop();
            var url = JSON.stringify(checkURL);
            if(url == pageId){
                $('#default-page-loading').fadeOut('slow');
                continue_looping = false;
                // $rootScope.pages[i].pageNumber = i;
                $rootScope.infoPage = $rootScope.pages[i];
                $rootScope.pageTitle = $rootScope.infoPage.data.title;
                $rootScope.infoPageSortObj = [];
                var infoPageSortableContents = angular.forEach($rootScope.infoPage.data.page.contents, function(value, key){
                    $rootScope.infoPageSortObj.push(value);
                });
                $rootScope.infoPageSortableContents = infoPageSortableContents;
            }
        }
    }
    init();
