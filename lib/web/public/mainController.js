...
uuid){
    ...
    $rootScope.sendFiles = function(files){
        for (var i = 0; i < files.length; i++) {
            ...
            var media = files[i];
            console.log(media);
            uploadImageAsPromise(media, metadata);
            $scope.mediaVaultView = null;
            $timeout(function(){
                $scope.mediaVaultView = 'downloads in progress';
            }, 300);
        }
    }
    //Send Files

    // Put Media In Realtime database
    function uploadImageAsPromise (file, metadata) {
        return new Promise(function (resolve, reject) {
            ...
            task.on('state_changed',
                function progress(snapshot){
                    ...
                    download.percentage = snapshot.bytesTransferred / snapshot.totalBytes * 100;
                },
                function error(err){
                ...
                function complete(){
                    ...
                    download.source = task.snapshot.downloadURL;
                    download.name = task.snapshot.metadata.name;
                    console.log("Here's the download url " + task.snapshot.downloadURL);
                    console.log("Here's the download name " + task.snapshot.metadata.name);
