...
filterFilter){
    ...
    $scope.updateMedia = function($index){
        ...
        mediaBucket.child($scope.updatedMedia.id).child("metadata").set(newMetadata);
        mediaBucket.child($scope.updatedMedia.id).once("value")
        .then(function(result){
            $scope.updatedMedia.cover = result.metadata.cover;
            $scope.updatedMedia.tags = result.metadata.tags;
            $scope.updatedMedia.source = result.source;
            angular.copy($scope.updatedMedia, $scope.imagelist[$scope.$index])
        });
    }
    ////////////////////////////////// Firebase Methods
            ...
            return new Promise(function (resolve, reject) {
                ...
                task.on('state_changed',
                    ...
                    function complete(){
                        ...
                        var storeMedia = mediaBucket.child(file.uploadID).set(file);
                    }
                );
            });
