...
image_data){
    ...
    $scope.addGalleryTemplate = function(){
        ...
        $scope.galleries.push(galleryObj);
    }
    $scope.addGallery = function(gallery){
        console.log("gallery added = " + JSON.stringify(gallery));
        var newGallery = {};
        newGallery.name = $scope.gallery.name;
        newGallery.description = $scope.gallery.description;
        newGallery.url = $scope.gallery.url;
        newGallery.featured = $scope.gallery.featured;
        newGallery.speed = $scope.gallery.speed;
        newGallery.tag = $scope.gallery.name;
        angular.forEach(newGallery.tag, function(tag) {
            return new Promise(function (resolve, reject) {
                //Upload tag
                var tagObj = {}
                tagObj.tag = tag;
        ...
        var storeGallery = galleriesDocument.child(gallery.id).set(newGallery);
        var galleryRef = galleriesDocument.child(gallery.id);
        $scope.galleries = [];
        galleryRef.once("value")
        .then(function(snapshot){
            var gallerySnapshot = snapshot.val();
            $scope.galleries.push(gallerySnapshot);
            console.log("Success! Here's the snapshot " + snapshot.val());
