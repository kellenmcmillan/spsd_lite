...
uuid){
    ...
    .then(function(snapshot) {
        ...
        console.log(taglist);
        $window.localStorage.setItem('tags', taglist);
    });

    var taglistWindowstorage = $window.localStorage.getItem('tags');
    
    $scope.$watch(taglistWindowstorage, function (newValue, oldValue, scope) {
        $scope.taglist = taglistWindowstorage;
    }, true);
