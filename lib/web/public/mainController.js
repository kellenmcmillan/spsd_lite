...
filterFilter){
    ...
    $scope.$on('server-event', function(event, args) {
        ...
        .textContent(args.data.message)
        .action('Close')
        .highlightAction(true)
        .highlightClass('primary-hn')// Accent is used by default, this just demonstrates the usage.
        .position("top right");
    ...
    //     snackbarContainer.MaterialSnackbar.showSnackbar(args.data.message);
    // });
    // listeners to broadcast on scope
    ...
    $scope.updateUser = function(user){
        ...
        if(user.personal_info.firstname){
            childDataValue.personal_info.firstname = user.personal_info.firstname;
        }
        if(user.personal_info.lastname){
            childDataValue.personal_info.lastname = user.personal_info.lastname;
        }
        if(user.personal_info.birthday){
            childDataValue.personal_info.birthday = user.personal_info.birthday;
        }
        if(user.personal_info.email){
            childDataValue.personal_info.email = user.personal_info.email;
        }
        if(user.personal_info.phone){
            childDataValue.personal_info.phone = user.personal_info.phone;
        }
    ...
    $scope.auth.$onAuthStateChanged(function(user) {
        if (user) {
            $rootScope.user = user;
            users_data.getMe($rootScope.user.id)
...
.factory('users_data', function($q){
    ...
    var getUsers = function(){
        ...
        .then(function(snapshot) {
            ...
            snapshot.forEach(function(childSnapshot) {
                ...
                childDataValue.personal_info.address = {};
                childDataValue.role = {};
                if(childData.personal_info.firstname){
                    childDataValue.firstname = childData.personal_info.firstname;
                }
                if(childData.personal_info.lastname){
                    childDataValue.lastname = childData.personal_info.lastname;
                }
                if(childData.personal_info.birthday){
                    childDataValue.birthday = childData.personal_info.birthday;
                }
                if(childData.personal_info.email){
                    childDataValue.email = childData.personal_info.email;
                }
                if(childData.personal_info.phone){
                    childDataValue.phone = childData.personal_info.phone;
                }
                if(childData.personal_info.address.street){
                    childDataValue.personal_info.address.street = user.personal_info.address.street;
                    childDataValue.personal_info.address.city = user.personal_info.address.city;
                    childDataValue.personal_info.address.state = user.personal_info.address.state;
                    childDataValue.personal_info.address.zipcode = user.personal_info.address.zipcode;
                }
                if(childData.role.alias){
                    childDataValue.role.alias = childData.role.alias;
                }
    ...
    var getMe = function(id){
        var defer = $q.defer();
        var user = {};
        usersBucket.child("users").child(id).once('value')
        .then(function(snapshot) {
            console.log("your user data received");
            var childDataValue = {};
            childDataValue.personal_info = {};
            childDataValue.personal_info.address = {};
            childDataValue.role = {};
