...
$mdExpansionPanel){
    ...
    var uploadIndex = 0;
    $scope.uploadIndex = uploadIndex;

    $scope.addUploadPreviewPanel = function(){
        $mdExpansionPanelGroup().waitFor('previewPanelGroup').then(function (instance) {
            if instance.count() > 0 {
                instance.collapseAll();
            }
            for (var i = 0; i < $scope.files.length; i++) {
                var panelName = 'panel' + i;
                instance.register(panelName);
                panelName.expand();
            }
        }
    }
    ...
    $scope.removeUploadPreviewPanel = function(panel) {
        $mdExpansionPanelGroup().waitFor('previewPanelGroup').then(function (instance) {
            var panelName = 'panel' + panel;
            instance.remove(panelName);
            if instance.count() > 0 {
                instance.collapseAll();
            }
        }
    ...
    $scope.removeAllUploadPreviewPanels = function () {
        $mdExpansionPanelGroup('previewPanelGroup').removeAll({animation: false});
    };
        ...
        return new Promise(function (resolve, reject) {
            ...
            task.on('state_changed',
                ...
                function complete(){
                    ...
                    var download = {};
                    download.url = task.snapshot.downloadURL;
                    download.name = task.snapshot.name;
                    console.log("image uploaded! Here's the download " + JSON.stringify(download));
                    console.log("Here's the download url " + download.url);
                    $scope.uploadedImages.push(download);
                    uploadIndex += 1;
                }
            );
        });
