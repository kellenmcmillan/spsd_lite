...
var main = angular.module('main', ['ngResource', 'ngAnimate', 'duScroll', 'appDataService', 'slideshowDataService'])

.controller('mainController', ['$rootScope', '$scope', '$compile', '$timeout', '$window', 
    '$interval', '$location', '$sce', '$anchorScroll', '$route', "$q", "$http", "frontend", "slideshow"
    function ($rootScope, $scope, $compile, $timeout, $window, $interval, $location, $sce, $anchorScroll, $route, $q, $http, frontend, slideshow){

    var view_data = frontend.query();
    var slideshow_data = slideshow.query();
    console.log(JSON.stringify(slideshow_data));
