...
$firebaseArray){
    ...
    function loginController($scope, $mdDialog, $firebaseAuth) {
    ...
    function signupController($scope, $mdDialog, $firebaseAuth, $firebaseArray) {
        ...
        $scope.createUser = function() {
            ...
            .then(function(user){
                // Add Successfully Signed Up User To User Database
                user.id = user.uid;
                storeUser(user);
                // Sign User Out After Adding To Database
    ...
    $scope.auth.$onAuthStateChanged(function(user) {
        ...
        if (user) {
            var firebaseUser;
            firebaseUser = userList.$getRecord(user.uid);
            firebaseUser.$loaded()
            .then(function(loadedUser) {
                $rootScope.user = loadedUser;
            })
            .catch(function(error) {
                console.log("Error:", error);
            });
            alert("here is the user " + user.email);
        } else {
            $rootScope.user = null;
            alert("Signed Out");
        }
