...
image_data){
    ...
    $scope.querySearch = function(query) {
        var results = [];
        results = query ? $scope.taglist.filter(createFilterFor(query)) : [];
        $scope.numberOfPages =  query ? Math.ceil(results.length/$scope.pageSize) : Math.ceil($scope.imagelist.length/$scope.pageSize);
        return results;
    }
