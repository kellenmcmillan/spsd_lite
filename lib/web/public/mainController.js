...
uuid){
    ...
    var taglist = [];
    tagBucket.on('value', function(snapshot) {
        ...
        console.log(taglist);
        $scope.$broadcast('taglist-change', {
            taglist: taglist
        });
    });
    
    $scope.$on('taglist-change', function(event, args) {
        console.log("here's the broadcast taglist " + args.taglist);
        $scope.taglist = JSON.stringify(args.taglist);
    });
