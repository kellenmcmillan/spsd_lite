...
gallery_data){
    ...
    $rootScope.openMediaVault = function() {
    ...
    // Galleries Document Init
    var galleriesDocument = realtimeDatabase.ref("galleries");
    // Galleries Document Init

    $scope.galleries = [];
    var newGalleryIndex = 0;

    // Retrieve Galleries From Database
    ...
    $scope.addGallery = function(gallery){
        ...
        var storeGallery = galleriesDocument.child(newGallery.id).set(newGallery);
        var galleryRef = galleriesDocument.child(newGallery.id);
    ...
    // Filter Media List By Tag
    $scope.filterByTag = function(){
        $scope.imagelist = [];
        if($scope.tagFilter){
            
            angular.forEach(imagelist, function(item){
                if (item.tags == $scope.tagFilter){
                    $scope.imagelist.unshift(item);
                }
            });
            console.log(JSON.stringify(result));
            console.log(result.length);
    ...
    $scope.updateMedia = function(image){
        var updatedImage = {};
        if(image.featured == null || undefined){
            image.featured = false;
        }
        updatedImage.featured = image.featured;
        if(image.tags == null || undefined){
            image.tags = null;
        }
        updatedImage.tags = image.tags;
        var storeGallery = firebaseStorage.ref().child(image.id).updateMetadata(updatedImage);
        var galleryRef = firebaseStorage.ref().child(image.id);
        galleryRef.once("value")
        .then(function(metadata){
            image.featured = metadata.featured;
    ...
    var getImages = function(){
        ...
        .then(function(snapshot) {
            ...
            snapshot.forEach(function(childSnapshot) {
                ...
                if(childData.tags){
                    childDataValue.tags = childData.tags;
                }
