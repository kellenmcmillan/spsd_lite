...
fromGalleriesDatabase){
    ...
    fromImageStorage.getImages().then(function(result){
        ...
        fromVideoStorage.getVideos().then(function(vid_result){
            if(vid_result.length > 0){
        ...
        fromGalleriesDatabase.getGalleries().then(function(result){
            ...
            for(let i = 0, l = $rootScope.galleries.length; i < l; i++) {
                if($rootScope.galleries[i].featured == true){
                    ...
                    var gallery_images = [];
                    // parse all images
                    for(let i = 0, l = $rootScope.medialist.length; i < l; i++) {
                        // to find images with the galleriy's tags
                        for(var key in $rootScope.medialist[i].metadata.tags) {
                            // push images with the gallery's tag into gallery's array of images
                            var tag = $rootScope.medialist[i].metadata.tags[key];
                            if(tag == gallery.tag){
                                gallery_images.push($rootScope.medialist[i]);
                            }
                        }
                    }
                ...
                for(let i = 0, l = $rootScope.medialist.length; i < l; i++) {
                    ...
                    for(var key in $rootScope.medialist[i].metadata.tags) {
                        ...
                        if(tag == gallery_object.tag){
                            if ($rootScope.medialist[i].metadata.cover == true){
                                gallery_objects_images.push($rootScope.medialist[i]);
                                gallery_object_compiled.cover = $rootScope.medialist[i].source;
                            } else {
                                gallery_objects_images.push($rootScope.medialist[i]); 
                            }
                        }
...
.factory('fromVideoStorage', function($q){
    var realtimeDatabase = firebase.database();
