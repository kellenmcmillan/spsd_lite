...
.controller('appLabController', ['$rootScope', '$scope', '$compile', '$window', '$interval', '$timeout', 'frontend', function ($rootScope, $scope, $compile, $window, $interval, $timeout, frontend){
    ...
    for(let i = 0, l = $rootScope.app_data.interior_page_configs.length; i < l; i++) {
        ...
        if(pages[i].name == "About SPSD"){
            var about_page = pages[i];
        } else if (pages[i].name == "SPSD Careers"){
            var careers_page = pages[i];
        } else if (pages[i].name == "SPSD Services"){
            var services_page = pages[i];
        }
    }
    // pulls relevant page data from appData Stream
