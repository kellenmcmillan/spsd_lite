...
filterFilter){
    ...
    $rootScope.updateStaffMember = function(id){
        var staffMember = $rootScope.editable_app_settings.staff[id];
        console.log(staffMember);
        var data = angular.toJson(staffMember);
        console.log("data " + data);
        var update = JSON.parse(data);
        var staffUpdate = staffBucket.child(id).set(update);
        $timeout(function(){
           $rootScope.$broadcast('server-event', {
                data:{
                    message: staffMember.name + " updated"
                }
            });
        }, 500);
    }

    $rootScope.removeStaffMember = function(id){
        var staffMember = $rootScope.editable_app_settings.staff[id];
        // remove user

        // remove user image

        // remove from auth database if present
    }
