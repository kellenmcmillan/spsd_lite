...
filterFilter){
    ...
    $rootScope.deletePage = function(page_index){
        if($rootScope.pages[page_index].settings.locked == false){
            $rootScope.pages.splice(page_index, 1);
            $timeout(function(){
               $rootScope.$broadcast('server-event', {
                    data:{
                        message: "Success. " + $rootScope.pages[page_index].settings.name + " removed."
                    }
                });
            }, 500);
        } else {
            $timeout(function(){
               $rootScope.$broadcast('server-event', {
                    data:{
                        message: "Failed to remove " + $rootScope.pages[page_index].settings.name
                    }
                });
            }, 500);
        }
