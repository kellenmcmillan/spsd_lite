	...
	gallery_data.getGalleries().then(function(result){
        ...
        for(let i = 0, l = $rootScope.galleries.length; i < l; i++) {
            ...
            if($rootScope.galleries[i].featured == true){
                console.log('featured gallery found!');
                var gallery = $rootScope.galleries[i];
                var gallery_images = [];
                // parse all images
                for(let i = 0, l = $rootScope.imagelist.length; i < l; i++) {
                    // to find images with the galleriy's tags
                    for(let i = 0, l = $rootScope.imagelist[i].metadata.tags.length; i < l; i++) {
                        // push images with the gallery's tag into gallery's array of images
                        if($rootScope.imagelist[i].metadata.tags[i] == gallery.tag){
                            console.log('image belonging to featured gallery found!');
                ...
                for (let i = 0, l = images.length; i < l; i++){
                    ...
                    if(images[i].cover == true){
                        console.log('featured gallery object being constructed!');
                        var gallery_package = {};
