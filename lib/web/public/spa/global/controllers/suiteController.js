...
filterFilter){
    ...
    var staffBucket = realtimeDatabase.ref('/appData/appSettings/staff/members');
    var newStaffIndex = 0;
    ...
    $rootScope.updateApp = function(element){
        switch(element){
            ...
            case "featuredProducts":
                ...
                break;
            case "infoPage":
                $rootScope.infoPage.data.page.contents = $rootScope.editable_pages.data.page.contents;
                var data = angular.toJson($rootScope.infoPage);
                var update = JSON.parse(data);
                appDataBucket.child("/pages/" + update.id).set(update);
                $timeout(function(){
                   $rootScope.$broadcast('server-event', {
                        data:{
