...
filterFilter){
    ...
    users_data.getUsers().then(function(result){
        if(result.length > 0){
            ...
            userslist = result;
            // for paginate
            $scope.usersPageSize = 20;
            $scope.currentUserPage = 0;
            $scope.numberOfUserPages = Math.ceil(result.length/$scope.usersPageSize);
        }
    });
    ...
    $scope.queryUser = function(query){
        ...
        $scope.currentPage = 0;
        $scope.userslist = filterFilter($scope.userslist,query);
        $scope.numberOfUserPages = Math.ceil($scope.userslist.length/$scope.pageSize);
    }
    $scope.clearQueryUser = function(){
        $scope.currentPage = 0;
        $scope.queryUserFilter = undefined;
    ...
    $scope.selectUser = function(user){
    	...
    	$scope.user_vault_detail_visible = true;
    }
    $scope.closeUserVaultDetail = function(){
    	$scope.selectedUser = null;
    	$scope.user_vault_detail_visible = false;
    }
    /////////////////  Open User Detail
...
.factory('role_data', function($q){
    ...
    // Storage Init
    var realtimeDatabase = firebase.database();
    var tagBucket = realtimeDatabase.ref().child('roles');
    // Storage Init
