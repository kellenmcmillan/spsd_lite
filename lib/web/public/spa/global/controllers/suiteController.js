...
filterFilter){
    ...
    $rootScope.startAnnouncementPageEdit = function(){
        $rootScope.edit_announcement_pages_dialog_visible = true;
    }
    $rootScope.endAnnouncementPageEdit = function(){
        $rootScope.edit_announcement_pages_dialog_visible = false;
    }
    $rootScope.startSpotlightPageEdit = function(){
        $rootScope.edit_spotlight_pages_dialog_visible = true;
    }
    $rootScope.endSpotlightPageEdit = function(){
        $rootScope.edit_spotlight_pages_dialog_visible = false;
    }
    ...
    $rootScope.addPage = function(type, data){
        ...
        switch(type){
            case "info":
                ...
                page_sample_data.settings.name = data.title;
            ...
            case "announcement":
                page_sample_data.data.title = data.title;
                page_sample_data.data.page.contents.push(paragraphObj);
                page_sample_data.settings.url = "/announcement/" + trim_title(data.title);
                page_sample_data.settings.page_type = "announcement";
                page_sample_data.settings.name = data.title;
                page_sample_data.settings.locked = false;
                $rootScope.pages.unshift(page_sample_data);
                $rootScope.app_pages.unshift(page_sample_data.settings);
            ...
            case "spotlight":
                ...
                page_sample_data.data.page.images.source = imageObj.source;
                page_sample_data.data.page.contents.push(paragraphObj);
                page_sample_data.settings.url = "/spotlight/" + trim_title(data.title);
                page_sample_data.settings.page_type = "spotlight";
                page_sample_data.settings.name = data.title;
                page_sample_data.settings.locked = false;
                $rootScope.pages.unshift(page_sample_data);
                $rootScope.app_pages.unshift(page_sample_data.settings);
    ...
    $rootScope.deletePage = function(page_index){
        ...
        if($rootScope.app_pages[page_index].locked == false){
            var page_name = $rootScope.app_pages[page_index].name;
            $rootScope.pages.splice(page_index, 1);
            $rootScope.app_pages.splice(page_index, 1);
            var data = angular.toJson($rootScope.pages);
            var update = JSON.parse(data);
            appDataBucket.child("pages").set(update);
            $timeout(function(){
               $rootScope.$broadcast('server-event', {
                    data:{
                        message: "Success. " + page_name + " removed."
                    }
                });
            }, 500);
        } else {
            $timeout(function(){
               $rootScope.$broadcast('server-event', {
                    data:{
                        message: "Failed to remove " + $rootScope.app_pages[page_index].name
                    }
                });
            }, 500);
        }
