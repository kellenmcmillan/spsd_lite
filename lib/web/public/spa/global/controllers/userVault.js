...
filterFilter){
    ...
    $rootScope.passwordReset = function(resetUser){

        var user = firebase.auth().currentUser;
        var newPassword = resetUser.resetPassword;
        var password_string = JSON.stringify(newPassword);
        console.log(newPassword + " " + password_string);
        user.updatePassword(password_string).then(function() {
            ...
            function(error){
                $timeout(function(){
                    $rootScope.$broadcast('server-event', {
                        data:{
                            message: "Failed to logout. Contact Lightweight."
                        }
                    });
                }, 500);  
            });
            ...
            $timeout(function(){
                $rootScope.$broadcast('server-event', {
                    data:{
                        message: "Failed to reset password. Contact Lightweight."
                    }
                });
            }, 500);
        });
