...
filterFilter){
    ...
    $rootScope.updateUser = function(user){
        var data = {
            address : {
                city : (user.address.city ? user.address.city : "BLANK"),
                state : (user.address.state ? user.address.state : "BLANK"),
                street : (user.address.street ? user.address.street : "BLANK"),
                zipcode : (user.address.zipcode ? user.address.zipcode : "BLANK")
            },
            birthday : (user.birthday ? user.birthday : "BLANK"),
            firstname : (user.firstname ? user.firstname : "BLANK"),
            lastname : (user.lastname ? user.lastname : "BLANK"),
            phone : (user.phone ? user.phone : "BLANK"),
            email : (user.email ? user.email : "BLANK"),
            security : {
                locked_out : (user.security.locked_out ? user.security.locked_out : false),
                roles : (user.security.roles ? user.security.roles : "BLANK"),
                security_question_1 : (user.security.security_question_1 ? user.security.security_question_1 : "BLANK")
            }
        }
    ...
    var getUsers = function(){
        ...
        .then(function(snapshot) {
            ...
            snapshot.forEach(function(childSnapshot) {
                ...
                var data = {
                    address : {
                        city : (childData.address.city ? childData.address.city : "BLANK"),
                        state : (childData.address.state ? childData.address.state : "BLANK"),
                        street : (childData.address.street ? childData.address.street : "BLANK"),
                        zipcode : (childData.address.zipcode ? childData.address.zipcode : "BLANK")
                    },
                    birthday : (childData.birthday ? childData.birthday : "BLANK"),
                    firstname : (childData.firstname ? childData.firstname : "BLANK"),
                    lastname : (childData.lastname ? childData.lastname : "BLANK"),
                    phone : (childData.phone ? childData.phone : "BLANK"),
                    email : (childData.email ? childData.email : "BLANK"),
                    security : {
                        locked_out : (childData.security.locked_out ? childData.security.locked_out : false),
                        roles : childData.security.roles,
                        security_question_1 : (childData.security.security_question_1 ? childData.security.security_question_1 : "BLANK")
                    },
                    id : childKey
                }
                usersList.push(data);
