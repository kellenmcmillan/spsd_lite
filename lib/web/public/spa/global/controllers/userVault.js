    ...
    $rootScope.selectUser = function(user){
    	$timeout(function(){
           $rootScope.$broadcast('server-event', {
                data:{
                    message: user.security.email + " Selected!"
                }
            });
        }, 500);
    ...
    $rootScope.updateUser = function(user){
        var data = {
            address : {
                ...
                state : (user.state ? user.address.state : null),
                street : (user.street ? user.address.street : null),
                zipcode : (user.zipcode ? user.address.zipcode : null)
            },
            ...
            security : {
                email : (user.security.email ? user.security.email : null),
