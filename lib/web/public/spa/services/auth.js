...
auth.service('auth', ['angularAuth0', '$timeout', '$location', 'authManager', 'jwtHelper', function(angularAuth0, $timeout, $location, authManager, jwtHelper){
    ...
    function setUser(authResult) {
		...
		let expiresAt = JSON.stringify((authResult.expiresIn * 1000) + new Date().getTime());
      	...
      	var token = jwtHelper.decodeToken(authResult);
      	
      	logged_in_user.profile = token;
      	logged_in_user.firstname = token.given_name || token.first_name;
      	logged_in_user.lastname = token.family_name || token.last_name;
    }
