...
.directive('minfullScreen', ['$window', '$compile', function($window, $compile){
  return{
    ...
    function link(scope, element, attrs){
      var headerHeight = 100;
      var screenHeight = $window.innerHeight - headerHeight;
      var maxScreenHeight = screenHeight + 200;
      element.css('min-height', screenHeight + 'px');
      element.css('max-height', maxScreenHeight + 'px');
      setTimeout(function(){
        attrs.$set('dataSize', element.height());
      }, 500);
      ...
      angular.element($window).bind('resize', function(){
        headerHeight = 100;
        screenHeight = $window.innerHeight - headerHeight;
        maxScreenHeight = screenHeight + 200;
        element.css('min-height', screenHeight + 'px');
        element.css('max-height', maxScreenHeight + 'px');
        setTimeout(function(){
          attrs.$set('dataSize', element.height());
...
.directive('missionScreen', ['$window', '$compile', function($window, $compile){
  return{
    link: link,
    restrict: 'A'
    };
    ...
    function link(scope, element, attrs){
      var headerHeight = 100;
      var screenHeight = $window.innerHeight - headerHeight;
      var elementHeight = angular.element('.mission-height').height();
      if (screenHeight > elementHeight){
		element.css('height', screenHeight + 'px');
	    setTimeout(function(){
	        attrs.$set('dataSize', elementHeight.height());
	    }, 500);
      } else {
      	element.css('height', screenHeight + 'px');
	    setTimeout(function(){
      ...
      angular.element($window).bind('resize', function(){
        headerHeight = 100;
        screenHeight = $window.innerHeight - headerHeight;
        elementHeight = angular.element('.mission-height');
        if (screenHeight > elementHeight){
			element.css('height', screenHeight + 'px');
		    setTimeout(function(){
		        attrs.$set('dataSize', elementHeight.height());
