...
$firebaseStorage)
{
    ...
    var load_slideshow = function(){
        ...
        for(let i = 0, l = $rootScope.all_galleries.length; i < l && continue_looping == true; i++) {
            console.log("looking for slideshow");
            var raw_name = $rootScope.all_galleries[i].name;
            var name = JSON.stringify(raw_url);
            // console.log("raw url " + raw_url);
            console.log("name stringified " + name);
            if(name.indexOf("Slideshow") > 0){
                console.log("found the slideshow!");
                ...
                slideshow_speed = item.transition;
            }
        }
