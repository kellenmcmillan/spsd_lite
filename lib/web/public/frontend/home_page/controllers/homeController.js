...
.controller('homeController', ['$rootScope', '$scope', '$compile', '$window', '$interval', '$timeout', function ($rootScope, $scope, $compile, $window, $interval, $timeout){
    ...
    var d = angular.element('.primary-blog-feed').height();
    
    $timeout(function(){
        var e; // C == Element Distance From Top
        var f; // D == Half Scroll Distance From Top == 50% Scroll Distance Over Time
        var g; // C == Element Distance From Top
        var h; // D == Half Scroll Distance From Top == 50% Scroll Distance Over Time
        var result_2;
        var result_3;
        e = a + b; // Sum Of Elements' Height
        f = e/2;
        result_2 = -1 * f;
        console.log("Result 2 = " + result_2);
        $scope.second_offset = result_2;
        g = a + b + c + d; // Sum Of Elements' Height
        h = g/2;
        result_3 = -1 * h;
    ...
    $(window).resize(function() {
        $timeout(function(){
            a = angular.element('.primary-parallax').height();
            b = angular.element('.primary-gallery').height();
            c = angular.element('.secondary-parallax').height();
            d = angular.element('.primary-blog-feed').height();
            e = a + b; // Sum Of Elements' Height
            f = e/2;
            result_2 = -1 * f;
            console.log("Result 2 = " + result_2);
