        ...
        if(gallery.is_featured == true){
            for(let c = 0, length = gallery.images.length; c < length; c++) {
                ...
                if(image.is_cover_image == true){
                    var image_package = {};
                    image_package.gallery_url_param = gallery._id;
...
.directive('contentAvailable', ['$location', '$timeout', function($location,$timeout){
    ...
    function link(scope, element, attrs){
        element.on("load", function(){
            var cancel_load_screen_timer = $timeout(function(){
               $('#default-page-loading').fadeOut('slow');
               $timeout.cancel(cancel_load_screen_timer);
            }, 10000);
            cancel_load_screen_timer();
            $('#default-page-loading').fadeOut('slow');
