...
.controller('homeController', ['$rootScope', '$scope', '$compile', '$window', '$interval', '$timeout', function ($rootScope, $scope, $compile, $window, $interval, $timeout){
    ...
    // $scope.third_offset = ($window.innerHeight * 2);
    $timeout(function(){
        var a = angular.element('.primary-parallax').height();
        var b = angular.element('.primary-gallery').height();
        var a1 = angular.element('.secondary-parallax').height();
        var b1 = angular.element('.primary-blog-feed').height();
        var c; // C == Element Distance From Top
        var d; // D == Half Scroll Distance From Top == 50% Scroll Distance Over Time
        var result;

        c = a + a1 + b + b1; // Sum Of Elements' Height
        d = c/2;
        result = -1 * (c + d);

        $scope.second_offset = b;
        $scope.third_offset = result;
    }, 1000);
    ...
    //     a + b = c; // Sum Of Elements' Height
    //     c/2 = d;
    //     var result = c + d;

    //     $scope.second_offset = angular.element('.primary-gallery').height();
    //     $scope.third_offset = $window.innerHeight + angular.element('.primary-gallery').height();
    // }, 1000);
