...
var home = angular.module('home', ['lightweightColor', 'scrollPosition', 'duScroll'])

.controller('homeController', ['$rootScope', '$scope', '$compile', '$window', '$interval', '$timeout', "$location", function ($rootScope, $scope, $compile, $window, $interval, $timeout, $location){
    ...
    $scope.our_team = page.our_team;
    $scope.featured_products = page.featured_products;
    ...
    parallax_timeout = $timeout(function(){
        ...
        $scope.secondary_parallax_bottom = $scope.secondary_parallax_top + angular.element('.secondary-parallax').height();
        ...
        $scope.featured_gallery_bottom = $scope.featured_gallery_top + angular.element('.primary-gallery').height() - 48;
        ...
        $scope.staff_container_top = $scope.map_container_bottom + 1;
        $scope.staff_container_bottom = $scope.staff_container_top + angular.element('.primary-staff').height();
        $scope.contact_container_top = $scope.staff_container_bottom + 1;
        ...
        var n; // C == Element Distance From Top
        var o; // D == Half Scroll Distance From Top == 50% Scroll Distance Over Time
        var result_1;
        var result_2;
        var result_3;
        var result_4;
        ...
        n = l + p + q; // Sum Of Elements' Height
        o = n/2;
        result_4 = -1 * o;
    ...
    $(window).resize(function() {
        parallax_timeout = $timeout(function(){
            $scope.slideshow_top = 0;
            $scope.slideshow_bottom = angular.element('.slideshow').height();
            $scope.mission_top = $scope.slideshow_bottom + 1;
            $scope.mission_bottom = $scope.mission_top + angular.element('.mission').height();
            $scope.secondary_parallax_top = $scope.mission_bottom + 1;
            $scope.secondary_parallax_bottom = $scope.secondary_parallax_top + angular.element('.secondary-parallax').height();
            ...
            $scope.primary_parallax_bottom = $scope.primary_parallax_top + angular.element('.primary-parallax').height();
            $scope.featured_gallery_top = $scope.primary_parallax_bottom + 1;
            $scope.featured_gallery_bottom = $scope.featured_gallery_top + angular.element('.primary-gallery').height();
            $scope.tertiary_parallax_top = $scope.featured_gallery_bottom + 1;
            $scope.tertiary_parallax_bottom = $scope.tertiary_parallax_top + angular.element('.tertiary-parallax').height();
            $scope.map_container_top = $scope.tertiary_parallax_bottom + 1;
            $scope.map_container_bottom = $scope.map_container_top + angular.element('.map').height();
        ...
        if(gallery.is_featured == true){
            for(let c = 0, length = gallery.images.length; c < length; c++) {
                ...
                if(image.is_cover_image == true){
                    ...
                    image_package.hi_res = image.hi_res;
