...
.controller('teamController', ['$rootScope', '$scope', '$compile', '$window', 'staff', function ($rootScope, $scope, $compile, $window, staff){
    ...
    .success(function(data) { 
        staff_data = JSON.stringify(data);
    }) 
   .error(function(err) { 
        console.log(err);
        return err; 
	...
	$scope.staffTiles = (function() {
		for (var i = 0; i < staff_data.data.lists.content.length; i++) {
			staff_member.push({
				avatar: staff_data.data.lists.content[i].avatar,
