...
.controller('teamController', ['$rootScope', '$scope', '$compile', '$window', 'staff', '$mdDialog', '$mdMenu', '$location', function ($rootScope, $scope, $compile, $window, staff, $mdDialog, $mdMenu, $location){
    ...
    .success(function(data) { 
        ...
        $scope.staffTiles = (function() {
          ...
          has_description: staff_data.data[0].content.lists[0].content[i].has_description,
   ...
   $scope.select_member = function(member){
    if(member.call_to_action !== null){
      $location.path(member.call_to_action);
    }
		$scope.member = member;
	}
