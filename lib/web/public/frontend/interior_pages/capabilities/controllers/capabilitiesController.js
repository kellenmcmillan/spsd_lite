...
.controller('capabilitiesController', ['$rootScope', '$scope', '$compile', '$window', '$routeParams', 'service_page_data', function ($rootScope, $scope, $compile, $window, $routeParams, service_page_data){
   ...
   .error(function(err) { 
    ...
    for(let i = 0, l = page.component_page_data.tabs.length; i < l; i++) {
    ...
    for(let i = 0, l = page.component_page_data.tabs.length; i < l; i++) {
        var paragraphs = page.data[0].content.tabs[i].text;
        var lists = page.data[0].content.tabs[i].lists;
        var anonymous_tab_variable = paragraphs.concat(lists);
        // Sort by position high to low
        page.data[0].content.tabs[i].compiled_content = anonymous_tab_variable.sort(sort_by('position', true, parseInt));
    }
