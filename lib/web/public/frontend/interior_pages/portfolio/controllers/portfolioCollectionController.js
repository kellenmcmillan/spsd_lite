...
.controller('portfolioCollectionController', ['$rootScope', '$scope', '$compile', '$location', '$timeout', '$window', '$anchorScroll', '$routeParams', function ($rootScope, $scope, $compile, $location, $timeout, $window, $anchorScroll, $routeParams){
    ...
    var slideshowEngineRev = function() {
        ...
        startValue++;
        if (slideshowStarting){
            startValue = starting_position;
            slideshowStarting = false;
            $scope.startValue = startValue;
            console.log('1st function start value is ' + startValue + ' slideshow length is ' + slideshow_length);
        } else if (startValue < slideshow_length){
            $scope.startValue = startValue;
            console.log('2nd function start value is ' + startValue);
        } else if (startValue = slideshow = slideshow_length){
            startValue = 0;
            $scope.startValue = startValue;
            console.log('3rd function start value is ' + startValue);
        } else {
            startValue = 0;
            $scope.startValue = 0;
            console.log('else function start value is ' + startValue);
        }
    }
...
.directive('lightboxImage', ['$window', '$location', '$timeout', function($window, $location, $timeout){
    ...
    function link(scope, element, attrs){
        element.on("load", function(){
            ...
            var lightbox_image = angular.element('.lightbox-image-size').height();
