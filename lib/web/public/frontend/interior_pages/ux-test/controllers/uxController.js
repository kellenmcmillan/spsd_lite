...
.controller('uxController', ['$rootScope', '$scope', '$compile', '$window', '$routeParams', '$mdDialog', '$mdMenu', function ($rootScope, $scope, $compile, $window, $routeParams, $mdDialog, $mdMenu){
	...
	$scope.settings = [
  	...
  	$scope.reorder_gallery = function (old_index, new_index) {
	    while (old_index < 0) {
	        old_index += $scope.image_gallery.length;
	    }
	    while (new_index < 0) {
	        new_index += $scope.image_gallery.length;
	    }
	    if (new_index >= $scope.image_gallery.length) {
	        var k = new_index - $scope.image_gallery.length;
	        while ((k--) + 1) {
	            $scope.image_gallery.push(undefined);
	        }
	    }
	    $scope.image_gallery.slice(new_index, 0, $scope.image_gallery.slice(old_index, 1)[0]);
	};
