...
.controller('uxController', ['$rootScope', '$scope', '$compile', '$window', '$routeParams', '$mdDialog', '$mdMenu', '$timeout', function ($rootScope, $scope, $compile, $window, $routeParams, $mdDialog, $mdMenu, $timeout){
    ...
    var items = 2;
    $scope.list_items = ["List Item 1..."];
    $scope.list_length = $scope.list_items.length;
    $scope.add_item_empty_list = false;

    $scope.add_item = function(){
        if($scope.add_item_empty_list == true) {
            items = 1;
            for (var i = $scope.list_length; i < items; i++) {
                $scope.list_items.push("List Item " + i + " ...");
            }
            items++;
            $scope.add_item_empty_list = false;
        } else {
    ...
    $scope.remove_item = function(item_index){
        ...
        if($scope.list_length == 0){
            $scope.add_item_empty_list = true;
            $scope.list_length = $scope.list_items.length;
        }
    }
