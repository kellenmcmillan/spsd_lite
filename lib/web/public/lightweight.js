...
uuid){
    ...
    image_data.getImages().then(function(result){
        ...
        video_data.getVideos().then(function(vid_result){
            if(vid_result > 0) {
                $rootScope.medialist = result;
                medialist = result;
                $rootScope.medialist = result.unshift(vid_result);
                medialist = result.unshift(vid_result);
                $rootScope.pageSize = 12;
                $rootScope.currentPage = 0;
                $rootScope.numberOfPages = Math.ceil($rootScope.medialist.length/$rootScope.pageSize);
            } else {
                $rootScope.medialist = result;
                medialist = result;
                $rootScope.pageSize = 12;
    ...
    $rootScope.queryMedia = function(query){
        $rootScope.currentPage = 0;
        $rootScope.medialist = filterFilter($rootScope.medialist,query);
        $rootScope.numberOfPages = Math.ceil($rootScope.medialist.length/$rootScope.pageSize);
    }
    $rootScope.clearQueryMedia = function(){
        $rootScope.currentPage = 0;
        $rootScope.queryFilter = undefined;
        $rootScope.medialist = medialist;
        $rootScope.numberOfPages = Math.ceil($rootScope.medialist.length/$rootScope.pageSize);
    }
    ...
    $rootScope.deleteMedia = function(image, index){
        ...
        firebaseStorage.ref(image_source).delete().then(function() {
            ...
            $timeout(function(){
                ...
                $rootScope.medialist.splice(index, 1);
            }, 500);
    ...
    $rootScope.sendImages = function(files){        
            ...
            return new Promise(function (resolve, reject) {
                ...
                task.on('state_changed',
                    ...
                    function complete(){
                        ...
                        $rootScope.medialist.unshift(file);
                    }
                );
            });
            $timeout(function(){
    ...
    $rootScope.sendVideo = function(file) {
        var metadata = {};
        metadata.name = file.name;
        return new Promise(function (resolve, reject) {
            var task = videoBucket.child(file.name).put(file, metadata);
            task.on('state_changed',
                function progress(snapshot){
                    file.percentage = snapshot.bytesTransferred / snapshot.totalBytes * 100;
                },
                ...
                function complete(){
                    var fileObj = {};
                    fileObj.metadata = {};
                    var appended_cloud_source = "https://storage.googleapis.com/spsd-189118.appspot.com/videos/"
                    fileObj.source = appended_cloud_source + task.snapshot.metadata.name;
                    fileObj.metadata.tags = file.tags;
                    fileObj.uploadID = uuid.v4();

                    var storeMedia = videoReferenceBucket.child(fileObj.uploadID).set(fileObj);
                    file.metadata = {};
                    file.source = fileObj.source;
                    file.filename = file.name;
                    file.metadata.tags = file.tags;
...
.factory('video_data', function($q){
    ...
    var getVideos = function(){
        var defer = $q.defer();
        var videolist = [];
        videoReferenceBucket.once('value')
        .then(function(snapshot) {
            snapshot.forEach(function(childSnapshot) {
                var childKey = childSnapshot.key;
                var childData = childSnapshot.val();
