...
module.exports = function lightweightRouter(app) {
	...
	function upload_media(request, response){
		...
		logger.log({ info:'Image Upload Fuction', message: 'Request Data ' + request});

        var buffer = request.image.buffer;
        var filename = request.fieldname + '-' + Date.now() + path.extname(request.data.originalname);
