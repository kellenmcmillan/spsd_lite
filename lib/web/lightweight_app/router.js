...
module.exports = function lightweightRouter(app) {
	...
	function upload_media(request, response){
		...
		var imageBuffer = decodeBase64Image(JSON.stringify(request.body));

        logger.log({ info:'Image Upload Successful', message: 'Image Buffer is ' + imageBuffer.data });
        fs.writeFile('/app/lib/web/public/assets/uploads/' + Date.now(), imageBuffer.data, function(err) {
            if (err) throw err
            logger.log({ info:'Image Upload', message: 'Successful'});
