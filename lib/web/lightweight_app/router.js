...
var storage = multer.diskStorage({ destination: '/app/lib/web/public/assets/uploads/'});
...
module.exports = function lightweightRouter(app) {
	...
	.post('/api/upload/media', upload.array('file', 1), upload_media)
	...
	function upload_media(request, response){

		logger.log({ info:'Image Upload Fuction', message: 'Request Data ' + request.file});
		var MAGIC_NUMBERS = {
		    jpg: 'ffd8ffe0',
		    jpg1: 'ffd8ffe1',
		    png: '89504e47',
		...
		upload(request, response, function(err) {
	        var buffer = request.file.buffer;
	        var magic = buffer.toString('hex', 0, 4);
	        var filename = request.data.file_1.uploaded_image + '-' + Date.now() + path.extname(request.file.originalname);
	        if (checkMagicNumbers(magic)) {
	            fs.writeFile('/app/lib/web/public/assets/uploads' + filename, buffer, 'binary', function(err) {
	                if (err) throw err
	                	logger.log({ info:'Image Upload Successful', message: 'filename is ' + filename })
	                response.end('File is uploaded');
	            })
	        } else {
	            response.end('File is no valid');
	        }
	    })
	}
