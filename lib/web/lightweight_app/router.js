...
var storage = multer.diskStorage({ destination: '/tmp/'});
...
module.exports = function lightweightRouter(app) {
	...
	function upload_media(request, response, next){
		...
		logger.log({ info:'Image Upload Fuction', message: 'Request Data ' + request.data.file.filename });

		var file = "/app/lib/web/public/assets/uploads/" + request.data.file.filename;
		  fs.rename(request.data.file.path, file, function(err) {
		    if (err) {
		    	logger.log({ info:'Image Upload Fuction', message: 'Problem With Image Upload ' + err });
		      console.log(err);
		      response.send(500);
		    } else {
		    	logger.log({ info:'Image Upload Fuction', message: 'Image Uploaded Successfully ' + request.data.file });
		      response.json({
		        message: 'File uploaded successfully',
		        filename: request.data.file.filename
		      });
		    }
		});
	}
