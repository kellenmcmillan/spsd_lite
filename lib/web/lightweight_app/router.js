...
module.exports = function lightweightRouter(app) {
	...
	.post('/api/upload/media', upload.single('uploaded_image'), upload_media)
	...
	function upload_media(request, response, next){
		...
		logger.log({ info:'Image Upload Fuction', message: 'Request Data ' + request.file.uploaded_image});

	    
		fs.writeFile(request.file.uploaded_image, function(err) {
            if (err) throw err
            logger.log({ info:'Image Upload Successful', message: 'filename is ' + request.file.uploaded_image.filename });
