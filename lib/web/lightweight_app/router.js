...
module.exports = function lightweightRouter(app) {
	...
	function upload_media(request, response, next){
		...
		logger.log({ info:'Image Upload Fuction', message: 'Request Data ' + request.file.filename });

		var file = "/app/lib/web/public/assets/uploads/" + request.file.filename;
		  fs.rename(request.file.path, file, function(err) {
		    if (err) {
		      ...
		      response.send(500);
		    	...
		    	logger.log({ info:'Image Upload Fuction', message: 'Image Uploaded Successfully ' + request.file });
		      response.json({
