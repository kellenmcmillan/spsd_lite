...
module.exports = function lightweightRouter(app) {
	...
	function upload_media(request, response){
		...
		logger.log({ info:'Image Upload Fuction', message: 'Request Data ' + JSON.stringify(request.body)});

        var buffer = request.body.image.buffer;
        var filename = request.body.fieldname + '-' + Date.now() + path.extname(request.body.image.originalname);
